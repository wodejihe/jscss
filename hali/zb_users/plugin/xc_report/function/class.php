...
$GLOBALS['table']['xc_report_list'] = '%pre%xc_report_list';     	  	 		
$GLOBALS['datainfo']['xc_report_list'] = array(     	   	  
    'ID'       => array('c_ID', 'integer','',0),      					 
    'Uid'      => array('c_Uid', 'integer','',0),      						
    'Aid'      => array('c_Aid','integer','',0),    	 						
    'Option'   => array('c_Option', 'string',250,''),       			  
    'Remarks'  => array('c_Remarks','string','',''),     	   			
    'Contact'  => array('c_Contact','string',250,''),       	 	  
    'Time'     => array('c_Time','integer','',0),         	  
    'Isdata'   => array('c_Isdata','string',50,''),       		 		
    'IP'       => array('c_IP', 'string', 200,''),    			     
    'Agent'    => array('c_Agent','string',250,''),    	  		  	
    'Meta'     => array('c_Meta','string','',''),     	  		  
);    	 	 	 		
      			 		
function xc_report_CreateTable_sql()    						  
{    		 		   
    global $zbp;    				 	 	
...
class xc_report_list extends Base{    	 			  	
     	 		 	 
    public function __construct()     	 			  	
    {     	 	 	 	
        global $zbp;    	    	  
      			 	 
        parent::__construct($zbp->table['xc_report_list'],$zbp->datainfo['xc_report_list'], __CLASS__);        	 		
    }       		  	
    ...
    {    	   	 		
        ...
        if($name == 'article'){    	  	  		
            $article = $zbp->GetPostByID($this->Aid);    	  	    
            if($article->ID < 1){    						  
                return '<span style="color:red;">'.$article->Title.'</span>';    			 	 		
            }    					   
            return '【'.($article->Status == 1 ? '草稿': ($article->Status == 2 ? '审核':'正常')).'】<a style="margin-right: 5px;" href="'.$article->Url.'" target="_blank">'.$article->Title.'</a><a href="'.$zbp->host.'zb_system/admin/edit.php?act=ArticleEdt&id='.$this->Aid.'" target="_blank" title="编辑"><i class="icon-pencil-square" style="font-size: 12px; color: red;"></i></a>';     		 	  	
        }       		 	 
    	 	  			
        if($name == 'articleEmail'){    	    		 
            ...
            return '【'.($article->Status == 1 ? '草稿': ($article->Status == 2 ? '审核':'正常')).'】<a href="'.$article->Url.'" target="_blank">'.$article->Title.'</a>';      				 	
        }      	 		  
        	  	
        if($name == 'Caozuo'){     		  		 
            return '<a style="margin-right: 20px;" href="javascript:see_log('.$this->ID.')">查看</a><a style="margin-right: 10px;color:red;" href="save.php?act=del_log&id='.$this->ID.'">删除</a>';    						 	
        }    			 		  
            		 	 	 	
        return parent::__get($name);        	 		
    }     	 			  
    	 			 		
    public function GetByList($w,$l = null,$op = null)     	  				
    {     		  	 	
        global $zbp;    	   	 	 
    	 	 		 	
        $sql = $this->db->sql->Select($this->table, array('*'),$w, array('c_ID'  => 'DESC' ),$l,$op);    	  	    
        $array = null;    		 		   
        $list = array();    	   		 	
        $array = $this->db->Query($sql);     		   	 
        if (!isset($array)) return array();     	   	 	
        foreach ($array as $a) {    	  	  	 
            $ls = new $this->classname;    			   		
            $ls->LoadInfoByAssoc($a);      			   
            $list[] = $ls;    		   			
        }        	 	 
...
class xc_report_vfimg{    	   	  	
    ...
    {    	 	     
        ...
        $this->height = 30;    	  	 			
        $this->fontsize = 20;    				 	 	
        $this->font = $zbp->path.'zb_system/defend/arial.ttf';    			     
    }     							
